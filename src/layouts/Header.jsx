			...
			>
				<div className="flex items-center gap-4 ">
					<div className="px-5 max-md:px-1" onClick={handleScrollToTop}>
						<Link to={"/"}>
							<h1 className="text-green-200 text-[35px] font-bold max-md:text-[20px] ">
								Sách
							</h1>
						</Link>
					</div>
					<ul className="flex gap-2 max-lg:fixed max-lg:left-0 max-lg:bottom-[-5px] max-lg:bg-[#1f1f1f] max-lg:p-5 max-lg:w-[100%] max-lg:justify-between max-md:p-2 items-center">
						<li className="text-white font-bold px-2 cursor-pointer max-md:text-[13px] max-md:py-3 max-md:px-0">
							<Link to={`/book/free`} className=" max-md:w-max">
								Sách miễn phí
							</Link>
						</li>
						...
						<li className="text-white font-bold px-2 cursor-pointer max-md:text-[13px] max-md:px-0">
							<Link to={`/book/suggest`}>Sách đề xuất</Link>
						</li>
						<li className="text-white font-bold px-2 cursor-pointer max-md:text-[13px] max-md:px-0">
							<Link to={`/book/retail`}>Sách bán chạy</Link>
						</li>
					</ul>
				</div>
						...
						{isUserLogin && isUserLogin ? (
							...
							<>
								<div className="relative ">
									...
									<div
										className={`flex gap-2 max-md:${useHiddenButton} max-md:absolute max-md:w-[200px] max-md:left-[-170px] top-[40px] max-md:flex`}
									>
										<button
											aria-label="Đăng ký"
											className="text-white max-md:bg-white max-md:text-[#139f7b] font-bold bg-[#C7C7C8] p-[6px] rounded-full border-[1px] border-gray-300 bg-transparent bg-opacity-50 w-[100px] 
                                        "
										...
										<button
											...
											className="text-white font-bold bg-[#139F7B] p-[5px] rounded-full w-[110px]"
