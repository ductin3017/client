...
const Product = () => {
	...
	const { showFormAddBookAdmin, showFormDescriptionBookAdmin } =
		useContext(DisplayPopup);
	const { showFormEditBookAdmin, handleDisplayEditBook, handleHideEditBook } =
		useContext(DisplayPopup);
				...
				dataBook.map(
					(book, index) =>
						editingProductId === book._id && (
							<div
								className=" fixed left-[35%] z-[10] top-[20px]"
								key={index._id}
							>
								<EditProductForm
									book={book} //
						...
						{dataBook.map((book, index) => (
							<tr className="border-b border-gray-200" key={index}>
								...
								<td className="text-left px-4 py-2">
									{FormatCurrency(book.labelBook && book.labelBook)}
								</td>
								<td className="text-left px-4 py-2">
									<img
								...
								<td className="text-left px-4 py-2">{book.genres[0].name}</td>
								<td className="text-left px-4 py-2">
									{" "}
									{categoryNames[book._id] ? (
										categoryNames[book._id]
									) : (
										<span>Đang tải...</span>
									)}
								...
								<td className="px-4 py-2">
									<div className="flex items-center justify-center">
										...
										/>
										<DeleteButton_square
											titleDelete="Xóa Sách"
											clickDelete={() => deleteBook(book._id)}
										/>
									</div>
								</td>
							</tr>
