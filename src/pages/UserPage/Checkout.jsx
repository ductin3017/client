...
const Checkout = () => {
	...
	const handleCheckUserCheckout = async () => {
		try {
			...
			if (nameMethodPayment === "QR Code") {
				window.location.href = `https://cdn.ttgtmedia.com/rms/misc/qr_code_barcode.jpg`;
			}
	...
	return (
		<>
			{errorEmpetyInfor && (
				<Error message={"Vui lòng cập nhật thông tin để thanh toán"} />
			)}
			{errorOption && (
			...
			{errorCartEmpety && (
				<Error message={"Vui lòng thêm sản phẩm rồi thanh toán"} />
			)}
			...
			<div className="w-full h-full">
				...
				<div className="pt-[150px] bg-[#191821] p-5 w-full h-auto max-lg:pt-[100px]">
					<h1 className="text-[#fff] text-[50px] font-bold max-lg:text-[30px] max-md:text-[20px]">
						Chọn hình thức thanh toán
					</h1>
					<span className="text-[#fff] font-semibold text-[20px] max-lg:text-[16px] max-md:text-[14px]">
					...
					{inforUser && inforUser === null ? (
						...
						<div className="flex justify-between mt-4 max-lg:flex-col max-lg:gap-3">
							<div className="w-1/2 max-lg:w-full">
								<MethodPayment
									ClickoptionQrCode={handleAtiveOptionPayment}
									borderActive={activeOption}
								/>
							</div>
